...
-- NRC Centers table
CREATE TABLE public.nrc_centers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  code TEXT NOT NULL UNIQUE,
...
CREATE TABLE public.fund_allocations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  transaction_hash TEXT UNIQUE, -- Simulated blockchain hash
  from_entity TEXT NOT NULL, -- State/District/Block
...
CREATE TABLE public.nrc_admissions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- RLS Policies for districts (readable by all authenticated users)
...
BEGIN
  ...
  -- Assign default role (public) to new users
